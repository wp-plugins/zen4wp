...
// TABLES: tax_rates, zones_to_geo_zones
  function zen4wp_get_tax_rate($class_id, $country_id = -1, $zone_id = -1) {
    ...
    $tax_query = "SELECT sum(tax_rate) as tax_rate
                  FROM (" . ZEN_TABLE_TAX_RATES . " tr
                  LEFT JOIN " . ZEN_TABLE_ZONES_TO_GEO_ZONES . " za ON (tr.tax_zone_id = za.geo_zone_id)
                  LEFT JOIN " . ZEN_TABLE_GEO_ZONES . " tz ON (tz.geo_zone_id = tr.tax_zone_id) )
                  ...
                  GROUP BY tr.tax_priority";  /*v1.3.0c*/

    $tax = $wpdb->get_results($tax_query, ARRAY_A);

    if (is_array($tax)) {
      $tax_multiplier = 1.0;
      foreach ($tax as $current_tax) {
        $tax_multiplier *= 1.0 + ($current_tax['tax_rate'] / 100);
      }
...
// Add tax to a products price based on whether we are displaying tax "in" the price
  function zen4wp_add_tax($price, $tax) {
    global $currencies;

    if ( (DISPLAY_PRICE_WITH_TAX == 'true') && ($tax > 0) ) {
      return $price + zen4wp_calculate_tax($price, $tax);
    } else {
      return $price;
    }
  }
 ...
 // Calculates Tax rounding the result
  function zen4wp_calculate_tax($price, $tax) {
    global $currencies;
